---
title: "オブジェクト指向で書く！テスタブルなソースコード！"
emoji: "🖋"
...
関数の組み合わせで作られているテストしにくい（テスタブルではない）プログラムから、オブジェクト指向で書かれたテストのしやすい（テスタブルな）プログラムにリファクタリングを行う中で、クラスを用いてプログラムを書くとどのような嬉しいことがあるのかについて説明させていただければと思います。

今回使用したソースコードなどは以下に置いてあります。ご自分で実行しながら読まれても面白いかもしれないです。
